...
const Results = ({savedImage, setSavedImage}: Props) => {
  ...
  return (
    <>
      {savedImage && (
        <div
          className="fixed top-0 left-0 w-full h-full z-50 bg-primary-100"
        >
          <button
          ...
          <div className="max-w-3xl mx-auto p-6 text-center">
            ...
            />
            <button type="button" onClick={saveImage}>画像を保存</button>
            <button type="button" onClick={shareImage}>シェア</button>
          </div>
        </div>
      )}
    </>
